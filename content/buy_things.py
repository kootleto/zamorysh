...
def read_menu_another(state=None):
    def tick_effect(gs):
        ui.display("Добро пожаловать в Другую кофейню!")
        ui.display("На данный момент в меню доступны следующие позиции:")
        ui.display("a. Латте | 8 кредитов")
        ui.display("b. Американо | 9 кредитов")
        ui.display("c. Капучино | 12 кредитов")
        if time.get_minute(gs) % 2 == 0:
            ui.display("d. Сырный латте с базиликом | 20 кредитов")
...
def buy_drink_another(hold_required=False, state=None):
    ...
    async def tick_effect(gs):
        ui.display("Введите букву позиции в меню + Enter")
        vv = await ui.prompt("Выбирайте: ")
        if vv == "a" and stats.get(gs, stats.MONEY) > 7:
            vitals.mod(gs, vitals.SLEEPINESS, -2)
            vitals.mod(gs, vitals.MENTAL, +15)
            stats.mod(gs, stats.MONEY, -9)
            ui.display(
...
def buy_drink_club(hold_required=False, state=None):
    ...
    async def tick_effect(gs):
        ui.display("Введите букву позиции в меню + Enter")
        vv = await ui.prompt("Выбирайте: ")
        if vv == "a" and stats.get(gs, stats.MONEY) > 24:
            vitals.mod(gs, vitals.SLEEPINESS, +15)
            vitals.mod(gs, vitals.MENTAL, +15)
...
def read_menu_club(state=None):
    def tick_effect(gs):
        ui.display("Добро пожаловать в клуб <<Nightlife>>!")
        ui.display("На данный момент в меню доступны следующие позиции:")
        ui.display("a. Коктейль <<Нулевая аффиксация>> | 25 кредитов")
        ui.display("b. Коктейль <<Singularia Tantum>> | 21 кредитов")
        ui.display("c. Виски | 20 кредитов")
        if time.get_minute(gs) % 2 == 0:
            ui.display("d. Коктейль <<Универсальная дробилка>> | 30 кредитов")
