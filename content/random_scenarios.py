...
def random_university_day_scenario(state=None):
    ...
    def f4(gs):
        ...
        ui.display_at(
            gs,
            "Вы искали что-то в Википедии и случайно перешли по ссылке на статью языка тигринья. "
...
def random_home_day_scenario(state=None):
    ...
    def f4(gs):
        ...
        ui.display_at(
            gs,
            "Вы уснули лицом в стол, хотя собирались делать домашку... "
            "Это негативно скажется на учёбе, но вы поспали.",
        )
    ...
    def f9(gs):
        vitals.mod(gs, vitals.SLEEPINESS, +10)
        ui.display_at(gs, "Вы чувствуете себя устало. Вам хочется поспать...")

    def f10(gs):
        ui.display_at(
            gs,
            "Вода из стиральной машины резко стала капать на пол. "
            "Вы долго вытирали её и так и не поняли, что случилось.",
        )
...
def random_park_day_scenario(state=None):
    ...
    def f5(gs):
        ui.display_at(gs, "Вы нашли траву и ПОТРОГАЛИ её.")
    ...
    def f7(gs):
        ui.display_at(
            gs,
            "Вода в парке очень красиво плещется. Вы засмотрелись на неё, и она убаюкала вас... ",
        )
    ...
    def f8(gs):
        ui.display_at(
            gs,
            "С вами завел беседу бомж. Вы рассказали ему о своей курсовой. Он расстроился и дал вам 5 кредитов.",
        )
        stats.mod(gs, stats.MONEY, +5)
    ...
    def f10(gs):
        ui.display_at(
            gs,
            "Солнце светило ярко, и вы решили поботать на природе... но ветер унёс листы из вашего конспекта по дискре!",
        )
...
def random_park_night_scenario(state=None):
    ...
    def f3(gs):
        ui.display_at(
            gs,
            "Вы уютно устроились на скамейке и не заметили, как задремали на несколько секунд. "
...
def random_metro_day_scenario(state=None):
    ...
    def f4(gs):
        ui.display_at(
            gs, "Вам всё ещё грустно из-за того, что ваш рюкзак досматривали."
        )
        vitals.mod(gs, vitals.MENTAL, -2)
