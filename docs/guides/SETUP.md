...
2. Создайте файл `.env` в корне проекта и скопируйте в него содержимое шаблона. В этом файле хранятся параметры
   конфигурации (например, `LOG_ENABLED`):

   ```bash
   cp .env.example .env
   ```

Теперь вы можете менять настройки проекта, редактируя значения в `.env`.
